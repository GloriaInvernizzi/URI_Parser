...
urilib_parse(URIString, URI) :-
    ...
    atom_codes(URIString, Codes),   % atoms_code � una funzione valida per coerce in prolog (casting)
    schema(Codes, SchemaCodes, AfterSchema),
    atom_codes(Schema, SchemaCodes), % trasforma lista in string, invesro, incognita in questo caso � Scheme
...
schema([C | Codes], Schema, After) :-
    ...
    !,
    Schema = [],  % Scheme � tutto quello che c'� prima del :, after � tutto quello che c'� dopo
    After = Codes.
