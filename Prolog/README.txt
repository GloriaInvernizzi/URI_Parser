...
La libreria viene eseguita tramite il predicato urilib_parse/2, che restituisce un URI scomposto a partire da una stringa contenente un URI in formato testuale; per fare ciò, viene prima riconosciuto lo schema utilizzato tramite il predicato schema/3, poi vengono impiegati diversi predicati, ognuno in grado di eseguire il parse di un singolo componente. 
...
Sono disponibili due predicati, urilib_display/1 e urilib_display/2, in grado di stampare su uno stream di destinazione. Nel caso di urilib_display/2, lo schema deve essere passato come argomento, mentre nel caso di urilib_display/1 viene usato lo stream corrente per richiamare urilib_display/2.